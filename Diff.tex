...
\begin{tikzpicture}[scale=0.9]
  \begin{axis}[
    ...
    \addplot[color=black,domain=3:5.5]{2*x-3.75};% draw lines of slopes M and -M through the point
    
    % yeah fix this fast please yeah
  \end{axis}
\end{tikzpicture}
\end{center}
Define $M = \sup_{(t,x) \in \mathcal{R}}|F(t,x)|$ (existence follows from EVT). Let $\eta \coloneqq \min(a,\frac{b}{M}) > 0$. Assume $M>0$; if $M=0$, then the IVP is $x' \equiv 0$, $x(t_0) = x_0$ has a solution: $x(t) \equiv x_0$ for all $t \in (t_0 - a,t_0 + a)$.
\end{document}
