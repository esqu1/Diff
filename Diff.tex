...
Revisiting the Picard sequence, we now know that all such $x_n \in C^0(I_{\eta})$. We also know that the Picard sequence is a cauchy sequence in $C^0(I_{\eta})$. We conclude that $\exists$ a limit function $x(t) = \lim_{n \to \infty}x_n(t)$ for all $t \in I_{\eta}$. 

Now we show that $x(t)$ solves our equation. \[ \int_{t_0}^t F(\tau, x_n(\tau)) \ d\tau \to \int_{t_0}^t F(\tau,x(\tau)) \ d\tau \]
Then, \[ 
\begin{aligned}
